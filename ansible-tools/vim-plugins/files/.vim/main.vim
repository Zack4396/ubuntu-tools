...
" Github: https://github.com/Hesanque/ubuntu-tools
"

" Load Once:
if exists('g:loaded_main_vim')
  finish
endif
let g:loaded_main_vim = 1

let s:my_vim_autoload = g:my_vim . "/autoload"
let s:my_vim_bundle = g:my_vim . "/bundle"

if !(exists('g:my_home') && isdirectory(g:my_home))
  echom " "
  echom "path " . g:my_home . " is not exist"
  echom " "
  echom "Please check the g:my_home parameter in your .vimrc setting"
  echom " "
  finish
endif
...
fu! s:prefix_plugin()
  ...
  " If you didn't install ctags, let's disable taglist/tagbar by default
  if !executable('ctags')
    let s:bypass_taglist = 1 
    let s:bypass_tagbar = 1 
  endif

  " If you didn't install global/gtags/gtags-cscope/htags, let's disable global by default
  if !executable('global') || !executable('gtags') || !executable('gtags-cscope') || !executable('htags')
    let s:bypass_global = 1
  endif
endf

" check plugin status
fu! s:check_plugin()
  call s:prefix_plugin()

  if !filereadable(s:my_vim_autoload . "/pathogen.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/winmanager/plugin/winmanager.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/nerdtree/plugin/NERD_tree.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/minibufexpl.vim/plugin/minibufexpl.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/taglist/plugin/taglist.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/tagbar/plugin/tagbar.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/lightline.vim/plugin/lightline.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/vim-airline/plugin/airline.vim")
 ...
 if !filereadable(s:my_vim_bundle . "/vim-airline-themes/plugin/airline-themes.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/vim-colors-solarized/colors/solarized.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/vim-colorschemes/README.md")
  ...
  if !filereadable(s:my_vim_bundle . "/ctrlp.vim/plugin/ctrlp.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/DrawIt/plugin/DrawItPlugin.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/global/plugin/gtags-cscope.vim")
  ...
  if !filereadable(s:my_vim_bundle . "/coc.nvim/autoload/coc.vim")
...
if !exists('g:loaded_pathogen')
  ...
  filetype plugin indent on               " plugin -> plugin on
                                          ...
                                          " on     -> detection on
  " FixSomething
  " If pathogen has been loaded youcompleteme/coc.nvim, let's disable youcompleteme by default
...
if exists('g:loaded_airline_themes')
  " More themes, please check s:my_vim_bundle . /vim-airline-themes/autoload/airline/themes
...
" --------------------------------------------------------------------------------------------------
if !exists('g:loaded_colors_solarized')
  set t_Co=256                        " tt
...
if !exists('g:loaded_colorschemes')
  " More themes, please check s:my_vim_bundle . /vim-colorschemes/colors
  "
...
if !exists('g:loaded_youcompleteme')
  let g:ycm_global_ycm_extra_conf = s:my_vim_bundle . "/youcompleteme/cpp/ycm/.ycm_extra_conf.py"
...
" --------------------------------------------------------------------------------------------------
if !exists('g:loaded_gtags') && !exists('g:loaded_gtags_cscope')
  " gtags-cscope
...
" -------------------------------------------------------------------------------------------------+
"if !(exists('s:bypass_drawit') && s:bypass_drawit)
if !exists('g:loaded_DrawItPlugin')
